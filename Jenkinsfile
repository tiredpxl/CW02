    ...
    stage('Deploy To Kubernetes') {
        ...
        script {
            withCredentials([sshUserPrivateKey(credentialsId: 'my-ssh-key', keyFileVariable: "KEY_FILE")]) {
                sh 'ssh -o StrictHostKeyChecking=no -i $KEY_FILE ubuntu@ec2-54-157-197-88.compute-1.amazonaws.com "kubectl set image deployments/coursework2 coursework2=sstark300/cw02:' + "${imageTag}" + '"'
            }
        }
    }
}
